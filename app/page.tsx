...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (platform: string) => {
    ...
    setError("");
    try {
      // Auto-provision or select first user for social login mock
      let { data: users, error: getError } = await supabase.from("user").select("*").limit(1);
      ...
      if (getError || !users || users.length === 0) {
        ...
        const res = await supabase.from("user").select("*").limit(1);
        users = res.data;
      }

      if (users && users.length > 0) {
        localStorage.setItem("smmhub_session", JSON.stringify(users[0]));
        router.push("/dashboard");
      } else {
        setError("Failed to query user credentials.");
        setIsLoading(false);
      }
    } catch (err: any) {
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.06] hover:border-white/20 transition-all duration-200 text-sm font-medium text-zinc-300 active:scale-[0.98] disabled:opacity-50 cursor-pointer"
              >
